...
gentoo_penguins_data_from_url <- read_csv(uri_gentoo)
```
...
theme_bw() +
scale_fill_viridis_d() +
  labs(title = "Culmen legth of Palmer Penguins",
       x = "Culmen legth",
       y = "# of Penguins")
```
...
```{r}
ggplot(data = penguins_data_from_csv, aes(x = culmen_depth_mm, y = body_mass_g, color= species, shape = island)) +
  geom_point() +
  theme_bw() +
  scale_color_viridis_d() +
...
```{r}
penguin_bm_speceis <- ggplot(data = penguins_data_from_csv, aes(x = body_mass_g, y = species, fill = species)) +
  geom_boxplot() +
  theme_bw() +
#wraping y column text
  scale_y_discrete(labels = function(y) str_wrap(y, width = 10)) +
  
  scale_fill_viridis_d() +
  labs(title = "Palmer Penguins",
       subtitle = "Bodymass of Species",
       x = "Body Mass",
       y = "Species")
penguin_bm_speceis
```
...
```{r}
ggplotly(penguin_bm_speceis)
```

How many penguins on Torgersen? can call using unique or using table function
...
There are 52 penguins on Togersen in our data set!

Now lets make a data frame that has the average culmen depth for each species.

```{r}
average_culmen_species <- penguins_data_from_csv %>% 
  group_by(species) %>% 
  summarise(average_culmen_species = mean(culmen_depth_mm, na.rm = TRUE))
```

now lets use the data frame #avg_culmen_species to plot the results

```{r}
ggplot(data = average_culmen_species, aes(x = species, y = average_culem_species)) + 
  geom_bar(stat = "identity") +
  

```

ggplot(data = average_species_body_mass, aes(x = species, y = average_body_mass)) + geom_bar(stat = "identity")
