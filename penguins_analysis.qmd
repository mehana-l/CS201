...
```{r}
avg_culmen_species <- penguins_data_from_csv %>% 
  group_by(species) %>% 
...
```{r}
ggplot(data = average_culmen_species, aes(x = species, y = average_culmen_species, fill = species)) +
  geom_bar(stat = "identity")+
  theme_bw() +
  scale_fill_viridis_d()+
  coord_flip() + 
  ...
  theme(legend.position = "none") +
  labs(title = "Average Culmen Length", 
       subtitle = "Palmer Penguins",
       x = "Species",
       y = "Culmen Length(mm)")
